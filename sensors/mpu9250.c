...
void mpu9250_get_data(I2C_Handle *i2c, float *ax, float *ay, float *az, float *gx, float *gy, float *gz) {
    ...
    int16_t GYRO_X = (rawData[8] << 8) | (rawData[9] & 0xff);
    int16_t GYRO_Y = (rawData[10] << 8) | (rawData[11] & 0xff);
    int16_t GYRO_Z = (rawData[12] << 8) | (rawData[13] & 0xff);
    ...
    //System_printf("accel x;%i y: %i z: %i temp: %i gyrox: %i y: %i z: %i \n", ACCEL_X, ACCEL_Y, ACCEL_Z, TEMP, GYRO_X, GYRO_Y, GYRO_Z);
