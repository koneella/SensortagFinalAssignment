...
void bmp280_get_data(I2C_Handle *i2c, double *pres, double *temp) {
    ...
    i2cTransaction.readCount = 6;
        ...
        if (I2C_transfer(*i2c, &i2cTransaction)) {
            ...
            uint8_t maski = 0xF0;
            uint8_t vali = maski & press_xlsb;
            uint8_t press_xlsb1 = vali >> 4;
    
            uint32_t valipaine = (press_msb << 12) | (press_lsb << 4) | (press_xlsb1);
            
            uint8_t temp_xlsb = rxBuffer[5];
            uint8_t temp_lsb = rxBuffer[4];
            uint8_t temp_msb = rxBuffer[3];
            
            uint8_t vali1 = maski & temp_xlsb;
            uint8_t temp_xlsb1 = vali1 >> 4;
    
            uint32_t valitemp = (temp_msb << 12) | (temp_lsb << 4) | (temp_xlsb1);
            ...
            *temp = bmp280_temp_compensation(valitemp);
            *pres = bmp280_convert_pres(valipaine);
            //System_printf("Lämpötila on: %i \n Paine on: %i \n", temp, press);
            // *pres = press;
            // *temp = tempp;
